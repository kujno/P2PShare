...
namespace P2PShare.Utils
{
    public class Elements
    {
        ...
        {
            string ipString = ip.ToString(), tag = "::ffff:";

            if (ipString.Contains(tag)) ipString = ipString.Substring(tag.Length);
        ...
        public static void ChangeFileTransferState(Send_ReceiveWindow sendReceiveWindow, int part, SendReceiveEnum receive_Send)
        {
        ...
        public static string Received_Sent(SendReceiveEnum receive_Send)
        {
            switch (receive_Send)
            {
        ...
        public static void FileTransferEndDialog(bool succeeded, Send_ReceiveWindow? sendReceiveWindow)
        {
            string message;
        ...
        private static void pickNIC(ComboBox nicComboBox, string nic)
        {
            if (nicComboBox.Items.Contains(nic)) nicComboBox.SelectedItem = nic;
        }
    }
}
